...
public class Laser : SingletonComponent<Laser> {
    ...
    public List<List<RCPoint>> UpdatePatterns1()
    {
        ...
        foreach (PATTERN patternType in patterns.Keys) {
            ...
            {
                LaserTaskBase currentPattern = patterns[patternType][patternID];
                // print(patternID);
                List<RCPoint> shapePoints = new List<RCPoint>();
                Vector2[] pointsPositions = currentPattern.NextPoints(Time.deltaTime);
                ...
                for (int pIdx = 0; pIdx < pointsPositions.Length; pIdx++)
                {
                    ...
                    if (MaskController.Instance.Masked(new Vector2(newPoint.x / 32767f, newPoint.y / 32767f)))
                    {
                        continue;
                    }
                    ...
                    if (isDashingColor)
                    {
                        ...
                        newPoint.red = (ushort)(currentPattern.brightness * rgb.x);
                        newPoint.green = (ushort)(currentPattern.brightness * rgb.y);
                        newPoint.blue = (ushort)(currentPattern.brightness * rgb.z);
                    }
                    else
                    {
                        newPoint.red = 0;
                    ...
                    if (currentPattern.dashLength != 0)
                    {
                        currDashLength += 1.0f / pointsPositions.Length;

                        if (currentPattern.dashLength <= currDashLength) //END OF DASH 
                        {
                    ...
                    newPoint.intensity = (ushort)(currentPattern.brightness);
                ...
                bool shouldDestroyPattern = currentPattern.brightness == 0;
